...
export default class Tile extends Component {
  constructor(props) {
    super(props);
    this.state = {
      width: this.props.gameVars.tileSize,
      height: this.props.gameVars.tileSize,
  ...
  debug() {
    const debug = this.props.gameState.debug.active;
    if (!debug) return;
    return (
      <span>
  ...
  render() {
    ...
    return (
      <DivTile ref={this.myRef} props={this.state} style={style}>
        {this.debug()}
      </DivTile>
    );
  }
